...
<script>
    ...
    function generateDataPenerbitTerbanyak() {
		$.ajax({
            url: '{{ url("searchval/penerbit-terbanyak") }}' + '?search=' + $('#keyword_pencarian').val() + '&filter_by=' + $('#filter_search').val() + '&jenis_media=' + $('#jenis_media').val(),
            type: 'GET',
            contentType: false,
            processData: false,
			//async :false,
    ...
    function generateDataKotaPenerbitTerbanyak() {
		$.ajax({
            url: '{{ url("searchval/kota-penerbit-terbanyak") }}' + '?search=' + $('#keyword_pencarian').val() + '&filter_by=' + $('#filter_search').val() + '&jenis_media=' + $('#jenis_media').val(),
            type: 'GET',
            contentType: false,
            processData: false,
			//async :false,
            success: function(response) {
                $('#top5kotapenerbit').empty();
                for (var d = 0; d < response.length; d++) {
    ...
    var navLinksPenerbit = document.querySelectorAll('#navbar-penerbit .nav-link');
    var navLinksKota = document.querySelectorAll('#navbar-kota .nav-link');
    function activateLink1() {
        navLinksPenerbit = document.querySelectorAll('#navbar-penerbit .nav-link');
    ...
    function activateLink2() {
        ...
        navLinksKota = document.querySelectorAll('#navbar-kota .nav-link');
    ...
    //link aktif untuk menu penerbit
        ...
        function setActiveLink(index) {
            ...
            if (clickedLink.classList.contains('active')) { //klik di aktif yang sama
                ...
                //dataTables();
                dataTables(filter2, keyword2,jenis_media2, false); //load data table
    ...
    //activateLink();
    function dataTables(params = null, keyword = null, jenis_media = null, generateSidebar = true) {
        if(generateSidebar) {
            generateDataKotaPenerbitTerbanyak();
            generateDataPenerbitTerbanyak();
        }
